...
function AppBar() {
  ...
  const changeTheme = () => {
    ...
    document.body.style.backgroundColor = newTheme ? 'white' : 'rgb(9,13,31)';

    const allTextElements = document.querySelectorAll('body *');

    allTextElements.forEach((element) => {
      if (element.tagName !== 'BUTTON') {
        element.style.color = newTheme ? 'black' : 'white';
      }
    });
  ...
  return (
    <div className="AppBar-Main">
      <h4>Sarkisyan Gevork</h4>

      <nav className="App-bar-texts">
        <h2>Blog</h2>
        <h2>Projects</h2>
        <h2>About</h2>
        <h2>Newsletter</h2>
        <button onClick={changeTheme} className="dark-light-theme-button">
          <svg
